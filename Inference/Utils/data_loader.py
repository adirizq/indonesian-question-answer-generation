...
from Utils.dataset import PipelineDataset, MultiTaskDataset, End2EndDataset
...
class End2EndQAGDataModule(pl.LightningDataModule):
    def __init__(self, model_type, dataset_csv_paths, dataset_tensor_paths, tokenizer, batch_size, recreate):

        super(End2EndQAGDataModule, self).__init__()
    ...
    def setup(self, stage=None):
        if stage == "fit":
            self.train_data = End2EndDataset(self.train_csv_path, self.train_tensor_path, self.tokenizer, self.recreate)
            self.valid_data = End2EndDataset(self.validation_csv_path, self.validation_tensor_path, self.tokenizer, self.recreate)
        elif stage == "test":
            self.test_data = End2EndDataset(self.test_csv_path, self.test_tensor_path, self.tokenizer, self.recreate)
