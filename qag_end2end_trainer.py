...
if __name__ == "__main__":
    ...
    model = QAGEnd2EndModel(pretrained_model, model_info[model_type]['type'], tokenizer, model_info[model_type]['lr_scheduler'], learning_rate)
    ...
    Path(f'./CSV Logs/End2End').mkdir(parents=True, exist_ok=True)
    Path(f'./Checkpoints/End2End').mkdir(parents=True, exist_ok=True)

    csv_logger = CSVLogger(f'CSV Logs', name=f'End2End/{model_type.value}')
    checkpoint_callback = ModelCheckpoint(dirpath=f'./Checkpoints/End2End/{model_type.value}', filename='{epoch}-{val_loss:.2f}', monitor='val_loss', mode='min')
    ...
    trainer = Trainer(
        ...
        default_root_dir=f'./Checkpoints/End2End/{model_type.value}',
    ...
    # Path(f'./Trained Model/End2End/{model_type.value}').mkdir(parents=True, exist_ok=True)
